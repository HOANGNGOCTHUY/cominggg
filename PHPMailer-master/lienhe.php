<?php
     if(isset($_POST['btn-contact']) == true){
          GuiMail();
     }
    ...
    try {
        ...
        $mail->addAddress('hoangngocthuy6@gmail.com', 'CÔNG TY TNHH ELSUN'); //mail và tên người nhận  
        $mail->isHTML(true);  // Set email format to HTML
        $mail->Subject = 'ĐĂNG KÝ NHẬN TIN/ THÔNG TIN LIÊN HỆ';
        $noidungthu = file_get_contents("./PHPMailer-master/noidungthulienhe.txt");
          $noidungthu = str_replace(
                         [ '{name}','{email}','{title}','{description}'], 
                         [$_POST['name'],$_POST['email'],$_POST['title'],$_POST['description']]
                         , $noidungthu);
